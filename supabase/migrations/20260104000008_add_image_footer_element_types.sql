-- Migration: Add image and footer element types to builder schema validation
-- This updates the validate_builder_schema function to include the new element types

-- Drop the constraint first
ALTER TABLE templates
DROP CONSTRAINT IF EXISTS check_builder_schema;

-- Update the validation function to include image and footer types
...
BEGIN
  ...
  LOOP
    ...
    -- Validate element type - now includes image and footer
    element_type := element->>'type';
    IF element_type NOT IN ('text', 'number', 'paragraph', 'dropdown', 'radio', 'date', 'calculated', 'divider', 'header', 'image', 'footer') THEN
      RETURN FALSE;
    END IF;
  END LOOP;
